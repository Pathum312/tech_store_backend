class CartService {
	...
	addToCart = async (req, res, next) => {
		try {
			...
			if (cart) {
				...
				this.logger.info(
					`200 - Cart Updated - /carts/add-to-cart - POST - ${JSON.stringify(req.body)}`,
				);
			...
			this.logger.info(
				`200 - Item added to cart - /carts/add-to-cart - POST - ${JSON.stringify(req.body)}`,
			);
	...
	destroyItem = async (req, res, next) => {
		try {
			...
			this.logger.info(
				`200 - Item Removed - /carts/delete-item - PATCH - ${JSON.stringify(req.body)}`,
			);
